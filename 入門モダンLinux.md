...
ログメッセージのスコープを小さく、具体的にしておくとメッセージを扱う側がコード内の場所を見つけるのが容易になる

ログはテキストで構成され、人間が読み、解釈してそれに基づき行動を決定する
...
* ログ、メッセージ、または行の集合体
  * 個別のイベントに関する情報を取得
* メタデータ、コンテキスト
  * メッセージごとに存在し、グローバルな範囲にも含まれる可能性がある
* 個々のログメッセージが解釈されるかの形式
  * ログの要素と意味を定義する、jsonなど

### 8.2.1 syslog

カーネル、デーモン、ユーザ空間まで、様々なソースのための標準的な路銀技機能

ヘッダ
* PRI = プライオリティ, ログの重要度を示す
* VER = プロトコルバージョン(省略される)
* TS = タイムスタンプ
* HN = ホスト名
* APP = アプリケーション名
* PID = プロセスID
...
実装はsyslogd,rsyslogd,syslog-ngなど
...
* 1つ、または複数のメトリクスを時系列などで追跡
* メトリクスの変化を監視し、閾値を超えた場合にアラートを生成する
...
* オートメーション(開発者としてのコードの追加が不要)
* カスタムインスツルメンテーション(コードのある時点でメトリクスを出力)

## 8.4 高度なオブザーバビリティ

### 8.4.1 トレースとプロファイリング

1台のマシンでプロセスの実効を時系列に沿ってキャプチャすること
...
pprof,Valgrindなどでプロファイリングを行う

parcaなどで継続的なプロファイリングが可能

### 8.4.2 PrometheusとGrafana

組み合わせて高度なオブザーバビリティが得られる
...
プロセスの停止や終了などのデフォルトの挙動がある

カスタムハンドラの指定も可能

シェルスクリプトでtrapコマンドを使うとシグナルハンドラを設定できる
...
KVM Kernel-based Virtual Machine カーネルに組み込まれている

VMM Virtual Machine Monitor VM管理、仮想デバイスをエミュレートする。QEMU,Firecrackerなど

ゲストカーネル、仮想マシン上で動作するカーネル

ゲストプロセス、ゲストカーネル上で実行するプロセス

### 9.2.1 KVM

Linux nativeな仮想化

コアモジュールはkvm.ko、intel,amdなどCPUアーキテクチャ依存のモジュールの2つがある。

I/O高速化のためVirtioなどのドライバもある

### 9.2.2 Firecracker

KVM仮想マシンを管理するVMM.Rustで実装されている

いわゆるmicroVMを管理し、ホストに対してHTTP APIを公開する
...
モダンなディストリビューションでは不変性に焦点を当てているものも多い

コンテナで完結するためパッケージマネージャが存在しないものも多い

## 9.4 セキュリティに関するトピック

### 9.4.1 Kerberos

中核となる考え方は、我々は通常安全でないネットワークを扱っているが、クライアントとサービスが互いに身元を証明するために安全な方法を求めている

* クライアントがKDCにリクエストを送り、サービスのクレデンシャルを要求
* 一時的な暗号化キーとサービスのチケットを応答
* クライアントはチケットをサービスに送信
* サービスはチケットをKDCに送信し、チケットが有効であることを確認
* サービスはクライアントに応答

KDCが単一障害点になる、時間要件が厳しい

### 9.4.2 PAM

PAM Pluggable Authentication Modules

具体的な認証スキームに依存しない、モジュール式のアーキテクチャで、認証、認可、アカウント管理、セッション管理などの機能を提供する

## 9.5 その他のモダンLinuxの話、あるいは将来の話

### 9.5.1 NixOS

パッケージ管理やシステム構成に関数型のアプローチをとっている
...
```
command &> hoge # 標準出力と標準エラー出力をhogeにリダイレクト
```
